...
class MyProductController extends Controller
{
    ...
    public function Draft()
    {
        if (Auth::check()) {
            $user = Auth::user();
            $myproducts = Product::where('user_id', $user->id)->where('status', 1)->get();
            $unproducts = Product::where('user_id', $user->id)->where('status', 5)->paginate(5);
    ...
    public function save(Request $request)
    {
        $this->validate($request, [
            'expiry_time' => 'required|integer|max:12',
        ...
        if ($input['used_for'] != null) {
            $input['used_for'] = $input['used_for'] . ' ' . $input['used'];
        }

        $input['warranty_period'] = $input['warranty_period'] . ' ' . $input['warranty'];
        if ($input['delivery'] == 1) {
            $input['delivery_area'] = null;
        }
    ...
    public function update($slug, Request $request)
    {
        ...
        if ($request->hasFile('featured_image')) {
            ...
            if ($myproduct->featured_image != null) {
