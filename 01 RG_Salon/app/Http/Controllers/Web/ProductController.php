...
class ProductController extends Controller
{
    ...
    public function index($slug)
    {
       ...
       $pro_price =json_encode($subcategories->product()->max('price'));
        ...
        {
            $all_subcategories=$subcategories->categories->first()->subcategories;
            ...
            if (request()->ajax()) {
                $view = view('web.product.ajaxView', compact('products', 'page','slug','sort','pro_price','all_subcategories'))->render();
                return response()->json(['html' => $view]);
            }
            return view('web.product.index',compact('products', 'page','slug','sort','pro_price','all_subcategories'));
        }
    ...
    public function changeStatus($product_code,Request $request)
    {
        $product=Product::where('product_code',$product_code)->where('status',3)->first();
